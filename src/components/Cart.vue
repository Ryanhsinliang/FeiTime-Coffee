<template>
  ...
  <Transition name="cart-slide">
    ...
    >
      <!-- Header -->
      <header
        class="flex items-center justify-between px-8 py-6 border-b border-[#e0e6e4]/60 bg-white/40"
      >
        <div class="flex items-center gap-3">
          <h2 class="text-2xl font-bold tracking-wide text-[#1a2e26] font-notoserif">您的購物車</h2>
          <span
            class="flex items-center justify-center h-6 min-w-[1.5rem] px-1.5 rounded-full bg-[#1a2e26] text-white text-xs font-bold shadow-sm"
          >
            {{ totalItems }}
          </span>
        </div>
      ...
      <div class="flex-1 overflow-y-auto no-scrollbar p-8 space-y-8">
        ...
        <div v-if="cartItems.length > 0" class="flex gap-5 items-start animate-fade-in-up">
          <div class="relative shrink-0 pt-2">
            ...
            <div
              class="group relative flex items-center justify-center w-11 h-11 rounded-full bg-white border border-[#DCCFC0] shadow-sm hover:shadow-md hover:border-[#8C7B5D]/40 transition-all duration-300 z-10 overflow-hidden p-1"
            >
              <svg
                viewBox="0 0 100 100"
                class="w-full h-full group-hover:scale-110 transition-transform"
              >
          ...
          <div
            class="relative flex-1 bg-white/95 backdrop-blur-sm border border-[#DCCFC0]/80 rounded-2xl rounded-tl-none p-5 shadow-sm group hover:shadow-md transition-shadow duration-300"
          >
            <div
              class="absolute -left-[7px] top-6 w-3.5 h-3.5 bg-white border-l border-b border-[#DCCFC0]/80 transform rotate-45"
            ></div>
            <div class="relative z-10">
              <div class="flex items-center gap-2 mb-2">
                <span
                  class="text-[11px] font-bold text-[#8C7B5D] uppercase tracking-wider flex items-center gap-1 font-notoserif"
                >
              ...
              <p class="text-sm text-[#1a2e26] leading-relaxed">
                絕佳選擇！
                <span class="font-bold border-b-2 border-[#8C7B5D]/30">
                  {{ featuredItem?.name || '耶加雪菲' }}
                </span>
                與您的口味檔案有
              ...
              <div class="mt-3 pt-3 border-t border-[#DCCFC0]/60 flex gap-2 items-start">
                <span class="material-symbols-outlined text-[#8C7B5D] text-[16px] mt-0.5">
                  lightbulb
                </span>
        ...
        <div v-if="recommendations.length > 0" class="pt-6 border-t border-[#e0e6e4]">
          <div class="flex items-center gap-2 mb-4 px-1">
            ...
            <h4 class="text-xs font-bold text-[#546e63] uppercase tracking-widest font-notoserif">
              您可能也喜歡
            </h4>
          </div>
          <RecommendationCard
      ...
      <div class="border-t border-[#e0e6e4] p-8 bg-[#eef2f0]/50 backdrop-blur-md">
        <div class="space-y-3 mb-6">
          ...
          <div class="flex justify-between items-center pt-4 border-t border-[#e0e6e4]/60">
            ...
            <span class="font-extrabold text-2xl text-[#1a2e26] font-notoserif">
              ${{ total.toFixed(2) }}
            </span>
          </div>
        </div>
