...
let lastLogTime = 0;
    ...
    // --- MATERIALS ---
    const woodMaterial = new THREE.MeshStandardMaterial({ color: '#4a3728', roughness: 0.7, metalness: 0.1 });
    const brassMaterial = new THREE.MeshStandardMaterial({ color: '#eebb44', metalness: 1.0, roughness: 0.15 });
    const leatherMaterial = new THREE.MeshStandardMaterial({ color: '#8B4513', roughness: 0.9 });
    const ceramicMaterial = new THREE.MeshStandardMaterial({ color: '#ffffff', roughness: 0.1, metalness: 0.1, flatShading: true });
    const paperMaterial = new THREE.MeshStandardMaterial({ color: '#fdfbf7', roughness: 1, side: THREE.DoubleSide });
    const groundCoffeeMaterial = new THREE.MeshStandardMaterial({ color: '#3b2f2f', roughness: 0.9 });
    const glassMaterial = new THREE.MeshPhysicalMaterial({ transmission: 1, thickness: 0.3, roughness: 0, clearcoat: 1, clearcoatRoughness: 0, ior: 1.5, color: '#ffffff' });
    const coffeeLiquidMaterial = new THREE.MeshStandardMaterial({ color: '#2a1810', roughness: 0.3, metalness: 0.1 });
    const kettleBodyMat = new THREE.MeshStandardMaterial({ color: '#2b2b2b', metalness: 0.7, roughness: 0.3 });
    const silverMat = new THREE.MeshStandardMaterial({ color: '#f0f0f0', metalness: 0.9, roughness: 0.2 });
    const waterDropMat = new THREE.MeshBasicMaterial({ color: '#aaddff', transparent: true, opacity: 0.8 });
    ...
    const initThree = () => {
      ...
      const connector = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.04, 0.2, 16), brassMaterial);
      ...
      const shell = new THREE.Mesh(new THREE.CylinderGeometry(1.1, 0.15, 1.6, 20, 1, true), ceramicMaterial);
      ...
      const filter = new THREE.Mesh(new THREE.CylinderGeometry(1.05, 0.1, 1.55, 32, 1, true), paperMaterial);
      ...
      const cBody = new THREE.Mesh(new THREE.CylinderGeometry(0.7, 1.2, 1.2, 32, 1, true), glassMaterial);
      ...
      const cNeck = new THREE.Mesh(new THREE.CylinderGeometry(0.75, 0.7, 0.3, 32, 1, true), glassMaterial);
      ...
      liquidInServerMesh = new THREE.Mesh(new THREE.CylinderGeometry(initTopR, 1.15, initHeight, 32), coffeeLiquidMaterial);
      ...
      const mBack = new THREE.Mesh(
        ...
        new THREE.MeshBasicMaterial({ color: 'white', opacity: 0.2, transparent: true, side: THREE.DoubleSide })
      );
      markingsGroup.add(mBack);
      [0.1, 0.3, 0.5].forEach(y => {
        const mark = new THREE.Mesh(new THREE.PlaneGeometry(0.2, 0.015), new THREE.MeshBasicMaterial({ color: 'white', opacity: 0.9 }));
      ...
      const tFace = new THREE.Mesh(new THREE.CircleGeometry(0.1, 32), new THREE.MeshBasicMaterial({ color: 'white' }));
      ...
      const kHandle = new THREE.Mesh(new THREE.TubeGeometry(handleCurve, 32, 0.06, 8, false), kettleBodyMat);
      ...
      const kSpout = new THREE.Mesh(new THREE.TubeGeometry(spoutCurve, 64, 0.05, 16, false), kettleBodyMat);
      ...
      // --- Particles ---
      dropsMesh = new THREE.InstancedMesh(new THREE.SphereGeometry(0.035, 8, 8), waterDropMat, MAX_DROPS);
      dropsMesh.frustumCulled = false;
      mainGroup.add(dropsMesh);

      // Steam
      const steamGeo = new THREE.SphereGeometry(0.2, 8, 8);
      const steamMat = new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.15, depthWrite: false });
      steamMesh = new THREE.InstancedMesh(steamGeo, steamMat, steamCount);
      mainGroup.add(steamMesh);

      // Floor
      const floor = new THREE.Mesh(new THREE.PlaneGeometry(100, 100), new THREE.MeshStandardMaterial({ color: '#f0e4d7', roughness: 0.8 }));
    ...
    const updatePhysics = () => {
      ...
      if (liquidInServerMesh) {
        liquidInServerMesh.geometry.dispose();
        liquidInServerMesh.geometry = new THREE.CylinderGeometry(currentTopRadius, RADIUS_BOTTOM, currentHeight, 32);
          ...
          options: {
            ...
            scales: {
              r: {
                ...
                pointLabels: { color: '#5d4037', font: { size: 10, weight: 'bold', family: "'LXGW WenKai TC', serif" } },
      ...
      if (lineCanvas) {
        lineChartInstance = new Chart(lineCanvas, {
          ...
          options: {
            ...
            plugins: { legend: { display: false } },
            scales: {
              x: { title: { display: true, text: '時間 (Time)', color: '#5d4037', padding: {top: 10} }, grid: { display: false }, ticks: { color: '#5d4037' } },
              y: { position: 'right', grid: { color: 'rgba(62, 39, 35, 0.05)' }, ticks: { color: '#5d4037' }, suggestedMax: 300 }
            }
          }
        });
      }
...
<template>
    <div class="relative w-full h-screen text-[#3e2723] select-none" style="font-family: 'LXGW WenKai TC', serif;">
        ...
        <div class="absolute inset-0 z-10 pointer-events-none flex flex-col justify-between p-4 md:p-8">
            ...
            <div class="absolute bottom-16 left-1/2 transform -translate-x-1/2 flex flex-col items-center gap-4 z-40 w-full max-w-md pointer-events-none">
                <div class="flex flex-col items-center drop-shadow-md">
                    <h1 class="text-xs tracking-[0.3em] uppercase mb-2 font-semibold text-white drop-shadow-[0_1px_2px_rgba(62,39,35,0.8)]">手沖模擬器 (Brew Simulator)</h1>
                    <div class="text-5xl font-light tracking-tighter tabular-nums font-mono text-white drop-shadow-[0_2px_4px_rgba(62,39,35,1.0)]">
                        {{ formattedTime }}
                    </div>
                </div>
            ...
            <div v-if="showResultModal" class="fixed inset-0 z-[60] flex items-center justify-center bg-[#3e2723]/20 backdrop-blur-sm pointer-events-auto transition-opacity duration-300 overflow-y-auto py-8">
                <div class="glass-strong p-8 rounded-3xl w-full max-w-lg flex flex-col gap-6 transform scale-100 animate-[fadeIn_0.3s_ease-out] mx-4 my-auto relative">
                    ...
                    <div class="flex justify-between items-center border-b border-[#3e2723]/10 pb-4">
                        ...
                        <button @click="showResultModal = false" class="text-gray-500 hover:text-[#3e2723] transition-colors">✕</button>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        ...
                        <div class="flex flex-col justify-center gap-4 text-[#3e2723]">
                            ...
                            <div>
                                <span class="text-xs text-[#8d6e63] uppercase tracking-widest">萃取率 (Extraction)</span>
                                <div class="text-xl font-mono" :class="Number(currentExtractionIndex) > 0 ? 'text-red-600' : 'text-blue-700'">
                                    {{ currentExtractionIndex }}
                                </div>
                            </div>
                            <div>
                                <span class="text-xs text-[#8d6e63] uppercase tracking-widest">均勻度 (Uniformity)</span>
                                <div class="text-xl font-mono text-purple-700">{{ currentUniformityIndex }}</div>
                            </div>
                        </div>

                        <!-- Radar Chart Clone Container -->
                        <div class="relative w-full aspect-square bg-[#3e2723]/5 rounded-xl p-2 flex items-center justify-center">
                            <canvas id="resultRadarChart"></canvas>
                        </div>
                    </div>

                    <!-- AI Report Section -->
                    <div class="mt-2 p-4 rounded-2xl bg-white/40 border border-white/50 backdrop-blur-md flex flex-col gap-3 min-h-[140px]">
                        <h3 class="text-sm font-bold text-indigo-700 flex items-center gap-2">
                              <div class="w-5 h-5"><CoffeeFairyIcon /></div>
                              咖啡小精靈的善意提醒
                        </h3>
                        
                        <div v-if="aiReport.loading" class="flex flex-col items-center justify-center py-6 gap-3 text-[#8d6e63]">
                            <div class="w-8 h-8 animate-bounce">
                                <CoffeeFairyIcon />
                            </div>
                            <span class="text-xs font-bold animate-pulse text-indigo-800">咖啡小精靈正在咕嚕咕嚕品嘗您傑作...</span>
                        </div>

                        <div v-else-if="aiReport.done" class="flex flex-col gap-3 animate-[fadeIn_0.5s_ease-out]">
                             ...
                             <div v-if="aiReport.top_issues.length > 0">
                                <span class="text-[10px] uppercase font-bold text-red-700/70 tracking-wider">主要問題 (Issues)</span>
                                <ul class="list-disc pl-4 mt-1 space-y-1">
                                    <li v-for="issue in aiReport.top_issues" :key="issue" class="text-xs text-red-900 leading-relaxed">
                                        {{ issue }}
                                    </li>
                                </ul>
                             </div>
                             ...
                             <div v-if="aiReport.next_attempt_plan.length > 0">
                                <div class="flex justify-between items-center mt-2">
                                    <span class="text-[10px] uppercase font-bold text-blue-700/70 tracking-wider">下次建議 (Next Plan)</span>
                                    <button @click="copyRecipe" class="text-[10px] px-2 py-0.5 rounded bg-blue-100 text-blue-700 hover:bg-blue-200 transition-colors">
                                        📋 複製 (Copy)
                                    </button>
                                </div>
                                <ol class="list-decimal pl-4 mt-1 space-y-1">
                                    <li v-for="step in aiReport.next_attempt_plan" :key="step" class="text-xs text-blue-900 leading-relaxed">
                                        {{ step }}
                                    </li>
                                </ol>
                             </div>
                        ...
                        <div v-else class="text-center py-4 text-xs text-[#8d6e63]">
                            等待報告生成...
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col gap-3 mt-2">
                        <div class="flex gap-3">
                            <button @click="downloadResult" class="flex-1 bg-[#3e2723] text-white py-3 rounded-xl font-bold hover:bg-[#5d4037] transition-colors flex justify-center items-center gap-2">
                                <span v-if="isSnapshotting">📸 處理中...</span>
                                <span v-else>📸 保存畫面 (Save)</span>
                            </button>
                            <button @click="copyShareLink" class="flex-1 glass py-3 rounded-xl font-bold hover:bg-[#3e2723]/5 transition-colors flex justify-center items-center gap-2 text-[#3e2723]">
                                <span>🔗</span> 複製連結 (Copy Link)
                            </button>
                        </div>
                        <button @click="resetSimulation" class="w-full py-2 text-sm text-[#8d6e63] hover:text-red-600 transition-colors">
                            重新開始 (Reset)
                        </button>
                    </div>
            ...
            <div class="flex flex-1 w-full md:justify-between items-start mt-20 md:mt-0 pt-10 px-0 md:px-4 transition-opacity duration-500 pointer-events-none" 
                ...
                <div class="flex flex-col gap-3 pointer-events-auto md:w-56">
                    ...
                    <div v-if="hasStarted" 
                        ...
                        <div class="flex justify-between items-baseline mb-2">
                            <span class="text-[9px] font-bold uppercase tracking-widest text-[#8d6e63]">沖煮指引 (Coach)</span>
                        ...
                        <div v-if="bloomPhase === 'wetting'">
                            ...
                            <div class="w-full bg-gray-200 h-1.5 rounded-full overflow-hidden">
                                <div class="bg-blue-500 h-full transition-all duration-100 relative" 
                        ...
                        <div v-if="bloomPhase === 'degassing'">
                            ...
                            <div class="w-full bg-gray-200 h-1.5 rounded-full overflow-hidden">
                                <div class="bg-yellow-500 h-full transition-all duration-100 relative" 
                        ...
                        <div v-if="aiAdvice.action" class="mt-2 pt-2 border-t border-blue-200/50 flex flex-col gap-1 animate-[fadeIn_0.5s_ease-out]">
                            ...
                            <div class="text-sm font-bold text-[#3e2723] leading-tight">{{ aiAdvice.action }}</div>
                            <div class="text-[10px] text-[#8d6e63] leading-tight opacity-90">{{ aiAdvice.reason }}</div>
                        </div>
